...
public class Transaction extends JFrame implements ActionListener{
    ...
    public Transaction(){
        ...
        buttons[1]= new JButton("CASH WITHDRAWAL");
    ...
    public void actionPerformed(ActionEvent ae){
        ...
        if(ae.getSource()==buttons[0]){ 
            ...
            if(pinn.equals("")){
                JOptionPane.showMessageDialog(null, "Please enter the PIN to continue");
            }
            else{
                try {
                    DatabaseConnection c1 = new DatabaseConnection();
                    ResultSet rs = c1.st.executeQuery("select balance from bank where pin = '"+pinn+"' order by(slno) desc limit 1");
