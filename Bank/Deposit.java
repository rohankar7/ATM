            ...
            if(ae.getSource() == this.b[0]){
                if(a.equals("")){
                    ...
                    if(rs.next()){
                        ...
                        String q1 = "select balance from bank where pin='"+b+"' and cardno='"+cardno+"' order by(slno) desc limit 1";
                        ResultSet rsBank = conn.st.executeQuery(q1);
                        // String pin = b;
                        if(rsBank.next()){
                            balance = rsBank.getDouble("balance");
                        }

                        Double credit = Double.parseDouble(a);
                        balance += credit;
                        conn.st.executeUpdate("insert into bank(cardno, pin, credit, debit, balance) values('"+cardno+"','"+b+"', '"+credit+"', null, '"+balance+"')");
